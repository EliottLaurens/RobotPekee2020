...
int main()
{
	...
	do
	{
		cout << "Chosissez un coefficient de distance pour le robot : ";
		cin >> coeffD;	
	}
	while(coeffD>0.5 || coeffD<0);
	do
	{
		cout << "Chosissez un coefficient de vitesse pour le robot : ";
		cin >> coeffV;		
	}
	while(coeffV>1 || coeffV<0);
	CRobotPekeeD robot("Pekee","localhost",coeffD,coeffV);
